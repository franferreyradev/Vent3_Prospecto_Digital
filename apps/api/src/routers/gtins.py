...
from src.core.deps import require_editor_or_admin
...
async def actualizar_gtin(
    ...
    request: Request,
    current_user: Usuario = Depends(require_editor_or_admin),
    session: AsyncSession = Depends(get_db),
) -> GtinRegistroResponse:
    service = GtinsService(session)
    gtin_registro = await service.actualizar(id, body, current_user, _ip_origen(request))
